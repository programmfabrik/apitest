...
- `--test some-single-test.json`: Run only some tests in the scope of a manifest. The path needs to be relative to the manifest file folder, so a single manifest should be specified using the `--single` option.
